...
const AddShows = () => {
  ...
  const { axios, getToken, image_base_url, user, fetchShows } = useAppContext()
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className="mt-8">
        ...
        <input type="text" id="trailer" value={url} onChange={(e) => setUrl(e.target.value)} className="inline-flex  items-center border border-gray-600  max-w-7xl mx-auto px-3 lg:w-2xl md:w-sm  py-2 rounded-md outline-none" />
      </div>
      {/* Show Price input */}
      <div className="mt-8">
        <label className="block text-sm font-medium mb-2" htmlFor="price">Show Price</label>
        <div className="inline-flex gap-2 border border-gray-600 px-3 py-2 rounded-md">
          <p className="text-gray-400 text-sm">{currency}</p>
          <input id="price" min={0} type="number" value={showPrice} onChange={(e) => setShowPrice(e.target.value)} placeholder="Enter show price" className="outline-none" />
        </div>
      </div>
