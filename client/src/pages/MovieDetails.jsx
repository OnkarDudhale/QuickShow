...
import { Dot, Heart, PlayCircleIcon, StarIcon } from 'lucide-react';
...
const MovieDetails = () => {
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-24 pt-30 '>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <p>{timeFormat(show.movie.runtime)} <Dot className="inline w-4 h-4" /> {show.movie.genres.map(genre => genre.name).join(", ")} <Dot className="inline w-4 h-4" /> {show.movie.release_date.split("-")[0]} </p>

          <div className='flex items-center flex-wrap gap-4 mt-4'>
            <button onClick={() => { navigate('/#trailer'); }} className='flex items-center gap-2 px-7 py-3 text-sm bg-gray-800 hover:bg-gray-900 transition rounded-md font-medium cursor-pointer active:scale-95'>
