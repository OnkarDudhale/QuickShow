...
const MovieCard = ({ movie }) => {
    ...
    const showSection = ['/upcoming', '/favourites'].includes(location.pathname)
    const favouriteSection = location.pathname === '/favourites'
    const upcomingSection = location.pathname === '/upcoming'
    ...
    return (
        <div className='relative flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-62'>
            {favouriteSection && <div className="w-7 absolute right-1 top-2"> <p className="bg-red-600 transition-all ease-in-out duration-300 rounded-full p-1 w-6 hover:scale-120 cursor-pointer flex justify-center items-center"><XIcon onClick={() => handleRemove(movie._id)} className=" h-4 w-4 text-white " /></p></div>}
            ...
            {!upcomingSection && <p className="mt-1">{movie.genres.slice(0, 2).map(genre => genre.name).join(' | ')} <Dot className="inline w-4 h-4" /> {timeFormat(movie.runtime)}</p>}

            {!showSection && <div className="flex items-center justify-between mt-4 pb-3">
